...
const LoginPage = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        if (result.success) {
            addToast({ message: 'Welcome back! You are now logged in.', type: 'success' });
            navigate('/');
        } else {
            addToast({ message: result.message || 'Login failed. Please try again.', type: 'error' });
        }
    };
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                    ...
                    <div>
                        <div className="flex justify-between items-center mb-2">
                            <label className="block text-[10px] font-bold tracking-widest text-gray-600 uppercase">
                                Password
                            </label>
                            <a href="/forgot-password" className="text-[10px] text-gray-500 hover:text-gray-900 transition-colors">
                                forgot password?
                            </a>
                        </div>
                        <div className="relative">
